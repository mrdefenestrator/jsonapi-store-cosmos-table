...
var CosmosTableStore = module.exports = function CosmosTableStore(config) {
  CosmosTableStore._checkMinServerVersion();
  this._config = config;
};
 ...
 */
CosmosTableStore.prototype.ready = false;


CosmosTableStore._checkMinServerVersion = function() {
...
CosmosTableStore._mongoUuid = function(uuid) {
  return new mongodb.Binary(uuid, mongodb.Binary.SUBTYPE_UUID);
};


CosmosTableStore._isRelationshipAttribute = function(attribute) {
  return attribute._settings && (attribute._settings.__one || attribute._settings.__many);
};


CosmosTableStore._toMongoDocument = function(resource) {
  var document = _.omitBy(resource, function(value) { return value === undefined; });
  document._id = CosmosTableStore._mongoUuid(document.id);
  return document;
};


CosmosTableStore._getRelationshipAttributeNames = function(attributes) {
  ...
  var relationshipAttributeNames = attributeNames.reduce(function(partialAttributeNames, name) {
    ...
    if (CosmosTableStore._isRelationshipAttribute(attribute)) {
...
CosmosTableStore._filterElementToMongoExpr = function(filterElement) {
...
CosmosTableStore.prototype._getSearchCriteria = function(request) {
  var self = this;
  var filter = request.processedFilter;
  if (!filter) return { };
  var criteria = Object.keys(filter).map(function(attribute) {
    var values = filter[attribute].map(CosmosTableStore._filterElementToMongoExpr);
...
CosmosTableStore._notFoundError = function(type, id) {
  return {
    status: "404",
    code: "ENOTFOUND",
...
CosmosTableStore._unknownError = function(err) {
  return {
    status: "500",
    code: "EUNKNOWN",
...
CosmosTableStore.prototype._createIndexesForRelationships = function(collection, relationshipAttributeNames) {
...
CosmosTableStore.prototype._applySort = function(request, cursor) {
...
CosmosTableStore.prototype._applyPagination = function(request, cursor) {
 ...
 */
CosmosTableStore.prototype.initialise = function(resourceConfig) {
  var self = this;
  if (!self._config.url) {
    return console.error("MongoDB url missing from configuration");
  }
  self.resourceConfig = resourceConfig;
  self.relationshipAttributeNames = CosmosTableStore._getRelationshipAttributeNames(resourceConfig.attributes);
 ...
 */
CosmosTableStore.prototype.populate = function(callback) {
  var self = this;
  if (!self._db) return;
 ...
 */
CosmosTableStore.prototype.search = function(request, callback) {
  var self = this;
  ...
  async.parallel({
    ...
    if (err) {
      return callback(CosmosTableStore._unknownError);
    }
 ...
 */
CosmosTableStore.prototype.find = function(request, callback) {
  var collection = this._db.collection(request.params.type);
  var documentId = CosmosTableStore._mongoUuid(request.params.id);

  debug("findOne", JSON.stringify({ _id: documentId }));
  collection.findOne({ _id: documentId }, { _id: 0 }, function(err, result) {
    if (err || !result) {
      return callback(CosmosTableStore._notFoundError(request.params.type, request.params.id));
    }
 ...
 */
CosmosTableStore.prototype.create = function(request, newResource, callback) {
  var collection = this._db.collection(newResource.type);
  var document = CosmosTableStore._toMongoDocument(newResource);
  debug("insert", JSON.stringify(document));
  collection.insertOne(document, function(err) {
    if (err) return callback(CosmosTableStore._unknownError(err));
 ...
 */
CosmosTableStore.prototype.delete = function(request, callback) {
  var collection = this._db.collection(request.params.type);
  var documentId = CosmosTableStore._mongoUuid(request.params.id);
 ...
 */
CosmosTableStore.prototype.update = function(request, partialResource, callback) {
  var collection = this._db.collection(request.params.type);
  var documentId = CosmosTableStore._mongoUuid(request.params.id);
  ...
  collection.findOneAndUpdate({
    ...
    if (err) {
      ...
      return callback(CosmosTableStore._unknownError(err));
    }
